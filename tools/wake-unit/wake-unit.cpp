...
#define CONVERTS_TO(x, y)\
EXPECT_EQUAL(y, convert_color(x))

#define CONVERTS_TO_DUMB(x, y)\
EXPECT_EQUAL(y, convert_color_dumb(x))

TEST(basic_foreground_colors) {
  //Black
...
TEST(basic_bold_foreground_colors) {
  //Black
...
TEST(basic_bold_foreground_colors_dumb) {
...
TEST(basic_bold_background_colors) {
  //Black
...
TEST(basic_bold_background_colors_dumb) {
...
TEST(basic_intense_foreground_colors) {
  //Black
...
TEST(basic_foreground_colors_dumb) {
...
TEST(basic_intense_background_colors) {
  //Black
...
TEST(basic_background_colors_dumb) {
...
TEST(rich_foreground_colors_dumb) {
  // Sweep test all 8-bit foreground colors
  for (int i = 0; i < 8; ++i) {
...
TEST(rich_background_colors) {
  // Sweep test all 8-bit foreground colors
  for (int i = 0; i < 8; ++i) {
    CONVERTS_TO("\033[48;5;" + std::to_string(i) + "m", "\033[" + std::to_string(40 + i) + "m");
  }
  for (int i = 8; i < 16; ++i) {
    CONVERTS_TO("\033[48;5;" + std::to_string(i) + "m", "\033[" + std::to_string(100 + i - 8) + "m");
  }
  for (int i = 16; i < 256; ++i) {
...
TEST(rich_background_colors_dumb) {
  // Sweep test all 8-bit foreground colors
  for (int i = 0; i < 8; ++i) {
...
TEST(dumb_gcc_smoke_test) {
  ...
  CONVERTS_TO_DUMB("\033[01m\033[Ktools/wake-unit/unit.h:146:95:\033[m\033[K \033[01;31m\033[Kerror:",
