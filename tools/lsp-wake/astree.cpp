...
void ASTree::recordComments(CSTElement def, int level) {
  if (def.id() == TOKEN_COMMENT) {
    ...
    std::string key = loc.filename + ":" + std::to_string(loc.start.row);
    comments[key] = Comment(def.segment().str(), def.location(), level);
  }
...
>>>>>>> f7809804 (Decrease doc annotation time from 0.5 sec to 0.005 sec)
...
void ASTree::fillDefinitionDocumentationFields() {
  ...
  for (size_t i = 0; i < definitions.size(); i++) {
    ...
    while (true) {
      ...
      std::string next_key = loc.filename + ":" + std::to_string(loc.start.row - 1);
      auto it = comments.find(next_key);
    ...
    if (doc_comments.size() == 1) {
      def.documentation = sanitizeComment(doc_comments.back().comment_text);
    }

    if (doc_comments.size() > 1) {
      def.documentation = sanitizeMultilineComment(doc_comments);
    }
    ...
    if (it != visited_definitions.end()) {
      ...
      // We need to delete the duplicate item. So we do an O(1) operation that
      // destroys order instead of an O(n) operation that maintains it
      definitions[i] = definitions.back();
      definitions.pop_back();

      // Rewind the loop and process the new element at index i
      i--;
    ...
    visited_definitions.insert({def.location, (definitions.data() + i)});
  }
}
...
std::string ASTree::sanitizeMultilineComment(const std::vector<Comment> &comment) {
  std::string composed;
  size_t size = comment.size();
  for (size_t i = 1; i <= size; i++) {
    composed += sanitizeComment(comment[size - i].comment_text) + "\n";
  }
  return composed;
}
