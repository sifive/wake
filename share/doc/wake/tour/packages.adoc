...
Thus, `from wake export map` will conflict with any other `def map` in the package.
...
from example export def YourTrue=MyTrue
from example export type YourBoolean=MyBoolean
----
...
Note that `from example export def MyTrue` would fail,
because it would re-introduce `MyTrue`
as a conflicting package-level identifier.
...
If no wake files are found, no package is selected.
If the wake files found share a common package, that package is selected.
Otherwise, no package is selected and one must be manually selected using '-p'.
