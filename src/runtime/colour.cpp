...
void TermInfoBuf::put(char c) {
  buf.sputc(c);
}

// Puts a string to `buf`
void TermInfoBuf::putstr(const char* str) {
...
static const char *cuu1 = nullptr;
static const char *cr = nullptr;
static const char *ed = nullptr;
static const char *sgr0 = nullptr;
...
const char *term_set_underline(bool should_underline)
{
  ...
  if (!sgr0) return "";
  if (should_underline) return wrap_termfn(tigetstr(underline));
  else return wrap_termfn(tigetstr(no_underline));
}
...
const char *term_set_standout(bool should_standout) {
  ...
  if (!sgr0) return "";
  if (should_standout) return wrap_termfn(tigetstr(standout));
  else return wrap_termfn(tigetstr(no_standout));
}
