...
PRIMTYPE(type_icmp) {
  return args.size() == 2 && args[0]->unify(Data::typeInteger) &&
         args[1]->unify(Data::typeInteger) && out->unify(Data::typeOrder);
}
...
PRIMTYPE(type_tget) {
  return args.size() >= 2 && args[0]->unify(Data::typeTarget) && args[1]->unify(TypeVar(FN, 2)) &&
         (*args[1])[0].unify(Data::typeTarget) && out->unify((*args[1])[1]);
}
