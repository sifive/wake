...
struct JAST {
  ...
  wcl::optional<int64_t> expect_integer(std::string key);
};
...
struct JSymbol {
  ...
  JSymbol(SymbolJSON type_, const Location &location_, std::string &&value_)
      : type(type_), location(location_), value(std::move(value_)) {}
};
