...
size_t TermStack::index(unsigned i) {
  TermStack *s = this;
  size_t idx, size;
  for (idx = i; idx >= (size = s->size()); idx -= size) s = s->next;
...
struct ToSSACommon {
  Runtime &runtime;
  TargetScope &scope;
  ToSSACommon(Runtime &runtime_, TargetScope &scope_) : runtime(runtime_), scope(scope_) {}
};
...
static void doit(ToSSACommon common, TermStack *stack, Expr *expr) {
  ...
  if (expr->type == &VarRef::type) {
    ...
    RFun *fun = new RFun(lambda->body->fragment,
                         lambda->fnname.empty() ? "anon" : lambda->fnname.c_str(), flags);
    ...
    for (auto &x : def->order) {
      ...
      if (common.scope[what->meta]->label.empty()) common.scope[what->meta]->label = x.first;
    }
    ...
    if (lit->litType == &Data::typeString) {
      lit->meta = common.scope.append(new RLit(
          std::make_shared<RootPointer<Value>>(String::literal(common.runtime.heap, lit->value))));
    } else if (lit->litType == &Data::typeRegExp) {
      lit->meta = common.scope.append(new RLit(
          std::make_shared<RootPointer<Value>>(RegExp::literal(common.runtime.heap, lit->value))));
    } else if (lit->litType == &Data::typeInteger) {
      lit->meta = common.scope.append(new RLit(
          std::make_shared<RootPointer<Value>>(Integer::literal(common.runtime.heap, lit->value))));
    } else if (lit->litType == &Data::typeDouble) {
      lit->meta = common.scope.append(new RLit(std::make_shared<RootPointer<Value>>(
          Double::literal(common.runtime.heap, lit->value.c_str()))));
    } else {
      assert(0);
    }
    ...
    for (unsigned i = con->cons->ast.args.size(); i > 0; --i) args.push_back(stack->index(i - 1));
    con->meta = common.scope.append(
        new RCon(std::shared_ptr<Constructor>(con->sum, con->cons), std::move(args)));
    ...
    std::vector<size_t> args;
    for (unsigned i = prim->args; i > 0; --i) args.push_back(stack->index(i - 1));
