...
struct DeepHash {
  ...
  bool operator()(const std::shared_ptr<RootPointer<Value> > &x,
                  const std::shared_ptr<RootPointer<Value> > &y) const {
    return (*x)->deep_hash(runtime->heap) == (*y)->deep_hash(runtime->heap);
  }
};

typedef std::unordered_map<std::shared_ptr<RootPointer<Value> >, size_t, DeepHash, DeepHash>
    ConstantPool;
...
void RArg::pass_inline(PassInline &p, std::unique_ptr<Term> self) {
  meta =
      make_meta(1, 0);  // we don't know the number unapplied args, but 0 prevents inlining anyway
  ...
  if (fnargs == args.size() - 1) {
    ...
    if (!term->get(SSA_RECURSIVE) && (singleton || meta_size(term->meta) < p.common.threshold)) {
      ...
      for (size_t i = 0; i <= n1; ++i)
        q.stream.discard(fargs[n1 - i],
                         fun->terms[i]->get(SSA_SINGLETON));  // bind arguments to inline function
      ...
      for (size_t i = fargs.size(); i > 0; --i) args.emplace_back(fargs[i - 1]);
...
void RDes::pass_inline(PassInline &p, std::unique_ptr<Term> self) {
  ...
  update(p.stream.map());
  bool same = true;
  for (unsigned i = 1; i < args.size() - 1; ++i) same &= args[0] == args[i];
  if (same) {
    ...
    if (input->id() == typeid(RCon)) {
      RCon *con = static_cast<RCon *>(input);
      std::unique_ptr<RApp> app(new RApp(args[con->kind->index], args.back(), label.c_str()));
      rapp_inline(p, std::move(app));
    } else {
      if (!input->get(SSA_ORDERED) && input->get(SSA_FRCON) && input->get(SSA_SINGLETON)) {
        RDes *des = static_cast<RDes *>(input);
        for (unsigned i = 0; i < args.size() - 1; ++i) p.stream[args[i]]->set(SSA_SINGLETON, false);
        ...
        for (unsigned i = 0; i < des->args.size() - 1; ++i) {
          ...
          RFun *f = new RFun(prior->fragment, prior->label.c_str(), 0, fnid + 3);
      ...
      bool known = true;
      for (unsigned i = 0; i < args.size() - 1; ++i) known &= p.stream[args[i]]->get(SSA_FRCON);
      set(SSA_FRCON, known);
    }
  }
}
...
void RFun::pass_inline(PassInline &p, std::unique_ptr<Term> self) {
  ...
  while (true) {
    ...
    terms.pop_back();
    ++ate;
    for (auto &x : child->terms) terms.emplace_back(std::move(x));
    output = child->output;
    label = child->label;
  }
...
std::unique_ptr<Term> Term::pass_inline(std::unique_ptr<Term> term, size_t threshold,
  ...
  for (auto &t : static_cast<RFun *>(term.get())->terms) t->set(SSA_SINGLETON, false);
